    ...
    <div className="min-h-screen bg-[#fafafa]">
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="w-full sm:w-4/5 md:w-3/4 lg:w-[94%] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center space-x-4">
          ...
          {!showForm && <button onClick={startCreate} className="ml-auto flex items-center px-4 py-2 bg-[#564c38] text-white rounded-lg hover:bg-[#695e46] transition-colors"><Plus className="w-4 h-4 mr-2" /> New Module</button>}
        </div>
      </header>
      <main className="w-full sm:w-4/5 md:w-3/4 lg:w-[94%] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {showForm ? (
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <h2 className="text-lg font-display font-semibold text-[#564c38]">{editingId ? 'Update Module' : 'Add a New Word List'}</h2>
            <p className="text-sm text-gray-600 mb-4">Give your word list a name, then add words one by one.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Word list title</label>
            ...
            <div className="border-t border-gray-200 pt-6">
              <h3 className="text-base font-display font-semibold text-[#564c38] mb-2">Words in this list</h3>
              <p className="text-sm text-gray-600 mb-4">Add each word using the form below. Only the Akan word and English meaning are required.</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Akan word</label>
                  <input value={newWord.akan} onChange={e => setNewWord({...newWord, akan: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">English meaning</label>
                  <input value={newWord.english} onChange={e => setNewWord({...newWord, english: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} required />
                </div>
                <div>
              ...
              <button type="button" onClick={addWord} className="px-4 py-2 bg-[#564c38] text-white rounded-lg hover:bg-[#695e46] transition-colors flex items-center"><Plus className="w-4 h-4 mr-2" /> Add Word</button>
            </div>

            {wordList.length > 0 && (
              <div className="mt-6">
                <h4 className="text-sm font-display font-semibold text-[#564c38] mb-2">Added words</h4>
