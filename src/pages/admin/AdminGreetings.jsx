...
const AdminGreetings = () => {
  ...
  const [form, setForm] = useState({ akan: '', english: '', pronunciation: '', context: '', audio_url: '', time_of_day: 'general', is_published: true });
  ...
  const startCreate = () => {
    setEditingId(null);
    setForm({ akan: '', english: '', pronunciation: '', context: '', audio_url: '', time_of_day: 'general', is_published: true });
    setShowForm(true);
  };

  const startEdit = (item) => {
    setEditingId(item.id);
    setForm({ id: item.id, akan: item.akan || '', english: item.english || '', pronunciation: item.pronunciation || '', context: item.context || '', audio_url: item.audio_url || '', time_of_day: item.time_of_day || 'general', is_published: item.is_published !== undefined ? item.is_published : true });
    setShowForm(true);
  };
    ...
    <div className="min-h-screen bg-[#fafafa]">
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="w-full sm:w-4/5 md:w-3/4 lg:w-[94%] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center space-x-4">
          ...
          {!showForm && <button onClick={startCreate} className="ml-auto flex items-center px-4 py-2 bg-[#564c38] text-white rounded-lg hover:bg-[#695e46] transition-colors"><Plus className="w-4 h-4 mr-2" /> New Greeting</button>}
        </div>
      </header>
      <main className="w-full sm:w-4/5 md:w-3/4 lg:w-[94%] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {showForm ? (
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <h2 className="text-lg font-display font-semibold text-[#564c38]">{editingId ? 'Update Greeting' : 'Add a New Greeting'}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Akan phrase</label><input value={form.akan} onChange={e => setForm({...form, akan: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} required /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">English meaning</label><input value={form.english} onChange={e => setForm({...form, english: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} required /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Pronunciation guide</label><input value={form.pronunciation} onChange={e => setForm({...form, pronunciation: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-2">Time of Day</label><select value={form.time_of_day} onChange={e => setForm({...form, time_of_day: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }}><option value="general">General</option><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option></select></div>
              <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-2">When to use this greeting</label><textarea value={form.context} onChange={e => setForm({...form, context: e.target.value})} rows={3} className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:ring-2 focus:border-transparent transition-shadow" style={{ '--tw-ring-color': '#564c38' }} /></div>
