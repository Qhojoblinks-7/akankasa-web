...
import { Search, Volume2, BookOpen, MapPin, Heart, Download, X } from 'lucide-react';
import AudioPlayer from '../components/AudioPlayer';
import { getFavorites, saveFavorites } from '../api';
            ...
            pages.push(
              <button
                ...
                className={`px-2 sm:px-3 py-2 rounded min-h-[44px] text-sm font-medium flex-shrink-0 ${
                  p === currentPage ? 'bg-[#564c38] text-white' : 'text-gray-700 hover:bg-gray-50'
                }`}
...
const Dictionary = () => {
  ...
  const toggleFavorite = (wordId) => {
    setFavorites((prev) => {
      ...
      const isFavorite = newFavorites.has(wordId);

      if (isFavorite) {
        newFavorites.delete(wordId);
        showSuccessToast('Word removed from favorites');
      } else {
        newFavorites.add(wordId);
        showSuccessToast('Word added to favorites');
      }

      const next = Array.from(newFavorites);
      saveFavorites(next);
      return next;
    });
  };
  ...
  const handleExport = () => {
    console.log('Exporting dictionary data...');
    showSuccessToast('Dictionary data exported successfully!');
  };
  ...
  const filteredWords = useMemo(() => {
    ...
    if (sortBy === 'alphabetical') {
      filtered.sort((a, b) =>
        searchDirection === 'akan-english' ? a.akan.localeCompare(b.akan) : a.english.localeCompare(b.english)
      );
    }
    return filtered;
      ...
      <div className="w-full max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 box-border overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md text-center">
            <div className="flex justify-center mb-3">
              <Heart className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-600 flex-shrink-0" />
            </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="divide-y divide-gray-200 lg:grid lg:grid-cols-3 overflow-hidden">
                    ...
                    <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 overflow-hidden">
                      <div className="flex-1 min-w-0 overflow-hidden">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3 sm:mb-4 overflow-hidden">
                          <h3 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight break-words min-w-0 overflow-hidden">
                            {word.akan}
                          </h3>
                          ...
                          <div className="overflow-hidden">
                            ...
                            <div className="flex flex-wrap gap-2 overflow-hidden">
                              {word.related.map((relatedWord, index) => (
                                <span
                                  key={index}
                                  className="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs sm:text-sm break-words flex-shrink-0"
                                >
                                  {relatedWord}
                                </span>
                              ))}
                      ...
                      <div className="flex flex-row lg:flex-col items-center gap-4 lg:gap-2 flex-shrink-0 overflow-hidden">
                        <button
                          ...
                          aria-label={`${favorites.includes(word.id) ? 'Remove' : 'Add'} ${
                            word.akan
                          } to favorites`}
                        >
            ...
            <aside
              className={`lg:col-span-1 border-t lg:border-t-0 lg:border-l border-gray-100 p-4 bg-gray-50 ${
                showFavorites ? 'block' : 'hidden lg:block'
              } overflow-hidden`}
            >
                      ...
                      <li key={fid} className="bg-white p-3 rounded shadow-sm overflow-hidden">
                        <div className="flex justify-between items-start gap-2 overflow-hidden">
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            <div className="text-xs sm:text-sm text-gray-500 leading-relaxed break-words">
                              {w.english}
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0 overflow-hidden">
